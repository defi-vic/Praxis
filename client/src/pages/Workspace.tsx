...
function InsightsView() {
  const dashboardQuery = trpc.insights.dashboard.useQuery();
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [intervention, setIntervention] = useState<any>(null);
  const interventionMutation = trpc.twin.intervention.useMutation({ onError: error => toast.error(error.message) });
  const insights: any[] = dashboardQuery.data?.insights ?? [];
  const selected = insights.find(insight => insight.id === selectedId) ?? insights[0];
  const createIntervention = async () => {
    if (!selected?.affectedStudents?.[0]) return;
    const student = selected.affectedStudents[0];
    setIntervention(await interventionMutation.mutateAsync({ studentId: student.studentId, concept: selected.concept, learningSignal: { correctness: student.mastery < 50 ? "needs_support" : "partial", mastery_estimate: student.mastery, confidence_signal: student.mastery < 50 ? "low" : "medium", misconception_detected: selected.issue, observation: selected.evidence, recommended_next_step: selected.recommendedAction, intervention_focus: selected.concept } }));
  };
  return <><div className="workspace-heading"><div><span className="micro-label">WORKSPACE / INSIGHTS</span><h1>Praxis noticed something.</h1><p>Prioritized learning signals translated into teacher-reviewed action.</p></div><span className="insight-live-badge"><i /> Live from student progress</span></div>{selected ? <section className="priority-insight"><div className="priority-insight-top"><div><span className="micro-label">MOST IMPORTANT CURRENT ISSUE · {selected.severity.toUpperCase()} SEVERITY</span><h2>{selected.issue}</h2><p>{selected.concept} · {selected.affectedStudents.length} affected student{selected.affectedStudents.length === 1 ? "" : "s"}</p></div><button className="workspace-primary" onClick={() => void createIntervention()} disabled={interventionMutation.isPending}>{interventionMutation.isPending ? "Creating…" : "Create intervention"} <ArrowUpRight size={14} /></button></div><div className="priority-evidence"><div><span className="micro-label">EVIDENCE</span><p>{selected.evidence}</p></div><div><span className="micro-label">RECOMMENDED TEACHER ACTION</span><p>{selected.recommendedAction}</p></div></div></section> : <div className="insight-empty">Learning signals will appear here after Praxis Twin evaluates a student response.</div>}<div className="insight-dashboard-grid"><section className="insight-list-card"><div className="panel-title"><div><span className="micro-label">PRIORITIZED INSIGHTS</span><h3>Where to intervene first</h3></div><span className="evidence-caption">Severity × students affected</span></div>{insights.length ? insights.map(insight => <button className={`prioritized-row ${insight.id === selected?.id ? "selected" : ""}`} key={insight.id} onClick={() => { setSelectedId(insight.id); setIntervention(null); }}><span className={`severity-dot ${insight.severity}`} /><span><strong>{insight.issue}</strong><small>{insight.concept} · {insight.affectedStudents.map((student: any) => student.name).join(", ")}</small></span><b>{insight.severity}</b><ArrowUpRight size={14} /></button>) : <p className="empty-copy">No prioritized insights yet.</p>}</section><section className="insight-list-card"><div className="panel-title"><div><span className="micro-label">LEARNING GAPS BY CLASS</span><h3>Classroom view</h3></div></div>{(dashboardQuery.data?.classGaps ?? []).map((gap: any) => <div className="class-gap-row" key={gap.classId}><div><strong>{gap.classId}</strong><span>{gap.concept}</span></div><b>{gap.averageMastery}%<small>average mastery</small></b><i style={{ width: `${gap.averageMastery}%` }} /><em>{gap.studentsNeedingSupport} may need support</em></div>)}<div className="misconception-summary"><span className="micro-label">COMMON MISCONCEPTIONS</span>{insights.slice(0, 4).map(insight => <div key={insight.id}><span>{insight.concept}</span><strong>{insight.affectedStudents.length} learner{insight.affectedStudents.length === 1 ? "" : "s"}</strong></div>)}</div></section></div><section className="recent-signals-card"><div className="panel-title"><div><span className="micro-label">RECENT LEARNING SIGNALS</span><h3>What Praxis Twin has learned</h3></div><span className="evidence-caption">Persisted evaluations</span></div>{(dashboardQuery.data?.recentSignals ?? []).length ? <div className="recent-signal-list">{dashboardQuery.data?.recentSignals.map((item: any) => <div key={item.id}><span className="signal-icon"><Sparkles size={13} /></span><span><strong>{item.concept}</strong><small>{typeof item.signal === "object" && item.signal?.observation ? item.signal.observation : "Twin evaluation recorded"}</small></span><em>{item.answer ? "Answer evaluated" : "Signal"}</em></div>)}</div> : <p className="empty-copy">No evaluated student responses yet. Open My Twin, ask Daniel to explain ATP, and submit a response to create the first learning signal.</p>}</section>{intervention && <section className="generated-intervention"><div><span className="micro-label">INTERVENTION · REVIEW BEFORE USE</span><h2>{intervention.title}</h2><p>{intervention.opening}</p></div><div className="generated-intervention-grid"><span><b>Teaching approach</b>{intervention.teaching_approach.join(" + ")}</span><span><b>Guided question</b>{intervention.guided_question}</span><span><b>Quick check</b>{intervention.check_for_understanding}</span><span><b>Target misconception</b>{intervention.expected_misconception}</span></div></section>}</>;
}
