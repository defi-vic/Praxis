    ...
    <main className="praxis-site">
      <header className="site-nav light-nav">
        <div className="container nav-inner">
          ...
          <nav className="desktop-nav"><button onClick={() => nav("how")}>How it works</button><button onClick={() => nav("dna")}>Teaching DNA</button><button onClick={() => nav("teacher")}>For teachers</button><button onClick={() => toast("Student experience coming soon")}>For students</button></nav>
          <div className="nav-actions"><Link className="nav-cta light-cta" href="/workspace">Enter Praxis <ArrowUpRight size={15} /></Link><button className="mobile-menu-button dark-menu" onClick={() => setMenuOpen(!menuOpen)} aria-label="Toggle menu">{menuOpen ? <X size={20} /> : <Menu size={20} />}</button></div>
        </div>
        {menuOpen && <div className="mobile-nav light-mobile">{[["how", "How it works"], ["dna", "Teaching DNA"], ["teacher", "For teachers"]].map(([id, label]) => <button key={id} onClick={() => nav(id)}>{label}<ArrowUpRight size={15} /></button>)}<button onClick={() => { setMenuOpen(false); toast("Student experience coming soon"); }}>For students <ArrowUpRight size={15} /></button><Link href="/workspace" onClick={() => setMenuOpen(false)}>Enter Praxis <ArrowUpRight size={15} /></Link></div>}
      </header>
